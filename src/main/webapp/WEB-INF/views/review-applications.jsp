...
<div class="container">
	<table class="table table-hover">
		<thead class="thead-light">
		  ...
		  <form action="${cp}/applications/${application.id}/process" method="post">
		  	<tr>
			  	<td>${application.applyDate}</td>
			  	<td><a href="${cp}/view-resume/${application.resume.id}">${application.resume.title}</a></td>		  	
<c:choose>
<c:when test="${application.status eq 'Pending Review'}">  
				<td>  			  
			  	<select name="status">
				...
				<td><input class="btn btn-secondary" type="submit" value="Submit"></td>
</c:when>
<c:otherwise>
